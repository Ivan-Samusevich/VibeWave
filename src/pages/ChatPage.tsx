...
import { Send, LogOut, User as UserIcon, RefreshCw } from 'lucide-react';
...
export const ChatPage: React.FC = () => {
  ...
  // Для примера используем ID собеседника = 2 (если мы 1) или 1 (если мы не 1)
  const targetUserId = user?.userId === 1 ? 2 : 1;

  const fetchMessages = async () => {
    if (!user?.userId) return;
    try {
      const data = await chatService.getMessages(targetUserId, user.userId);
  ...
  useEffect(() => {
    ...
    // Простой поллинг для имитации реального времени (раз в 5 секунд)
