...
import { User, Grid, Bookmark, Tag, MessageCircle, UserPlus, UserCheck, Heart, Camera, X, Calendar } from 'lucide-react';
            ...
            <div className="flex-1 space-y-5 text-center md:text-left">
                ...
                <div className="flex items-center justify-center md:justify-start gap-3">
                  {isOwnProfile ? (
                    ...
                    <>
                      ...
                      >
                        {profileData.isFollowing ? (
                          <span className="flex items-center gap-2 font-bold"><UserCheck className="h-4 w-4" /> Вы подписаны</span>
                        ) : (
                          <span className="flex items-center gap-2 font-bold cursor-pointer"><UserPlus className="h-4 w-4" /> Подписаться</span>
                        )}
                      </Button>
                      <Button 
                        variant="secondary" 
                        ...
                        onClick={() => navigate(`/chat?user=${profileData.userName}`)}
                      >
              ...
              <div className="grid grid-cols-3 md:flex md:gap-10 text-sm py-2 border-y md:border-none border-zinc-100 dark:border-zinc-800">
                <div className="flex flex-col md:flex-row md:gap-1.5 items-center">
                  <span className="font-bold text-lg md:text-base text-zinc-900 dark:text-zinc-100">{profileData.postCount || 0}</span>
                ...
                <div className="flex flex-col md:flex-row md:gap-1.5 items-center cursor-pointer group">
                  <span className="font-bold text-lg md:text-base text-zinc-900 dark:text-zinc-100 group-hover:text-indigo-600 transition-colors">{profileData.followerCount || 0}</span>
                  <span className="text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-700 dark:group-hover:text-zinc-300">подписчиков</span>
                </div>
                <div className="flex flex-col md:flex-row md:gap-1.5 items-center cursor-pointer group">
                  <span className="font-bold text-lg md:text-base text-zinc-900 dark:text-zinc-100 group-hover:text-indigo-600 transition-colors">{profileData.followingCount || 0}</span>
                  <span className="text-zinc-500 dark:text-zinc-400 group-hover:text-zinc-700 dark:group-hover:text-zinc-300">подписок</span>
                </div>
              </div>
          ...
          userPosts.map((post, i) => (
            ...
            >
              ...
              <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 text-white font-bold gap-6 transition duration-300">
                <div className="flex items-center gap-2"><Heart className="h-5 w-5 fill-white" /> {post.likesCount}</div>
              </div>
            </motion.div>
          ))
        )}
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between mb-6">
                <h2 className="text-xl font-bold dark:text-zinc-100">Редактировать профиль</h2>
                <button onClick={() => setIsEditModalOpen(false)} className="text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 cursor-pointer">
                  <X className="h-6 w-6" />
                </button>
              </div>
              ...
              <div className="space-y-6">
                <div className="flex flex-col items-center gap-4">
                  <div className="relative h-24 w-24 rounded-full overflow-hidden bg-zinc-100 dark:bg-zinc-800 border-2 border-zinc-200 dark:border-zinc-700">
                    {editPreview || profileData.avatarUrl ? (
                      <img src={editPreview || profileData.avatarUrl} className="w-full h-full object-cover" />
                    ) : (
                  ...
                  <p className="text-xs text-zinc-500 font-medium whitespace-nowrap">Нажмите на фото, чтобы изменить аватар</p>
                </div>

                <div className="space-y-2">
                  <label className="text-sm font-semibold dark:text-zinc-300">О себе</label>
                  <textarea
                    autoFocus
                ...
                <div className="flex gap-3 pt-2">
                  <Button variant="outline" className="flex-1 cursor-pointer" onClick={() => setIsEditModalOpen(false)}>Отмена</Button>
                  <Button className="flex-1 bg-indigo-600 hover:bg-indigo-700 cursor-pointer" onClick={handleUpdateProfile}>Сохранить</Button>
                </div>
              </div>
            </motion.div>
          </div>
            ...
            >
              ...
              <div className="flex-1 bg-black flex items-center justify-center min-h-300px">
                {selectedPost.fileType === 'video' ? (
                  <video src={selectedPost.imageURL} controls className="max-h-full max-w-full" />
                ) : (
                  <img src={selectedPost.imageURL} className="max-h-full max-w-full object-contain" />
                )}
              </div>

              {/* Info Part */}
              <div className="w-full md:w-400px flex flex-col bg-white dark:bg-zinc-900">
                <div className="p-4 border-b border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="h-10 w-10 rounded-full bg-linear-to-tr from-indigo-500 to-purple-500 p-0.5">
                      {profileData.avatarUrl ? (
                        <img src={profileData.avatarUrl} className="h-full w-full rounded-full object-cover border-2 border-white dark:border-zinc-900" />
                      ) : (
                    ...
                    <span className="font-bold text-sm dark:text-zinc-100">{selectedPost.userName}</span>
                  </div>
                  <button onClick={() => setIsPostModalOpen(false)} className="p-2 hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-full transition-colors">
                ...
                <div className="flex-1 p-4 overflow-y-auto space-y-4">
                  <div className="flex gap-3">
                    <div className="h-8 w-8 rounded-full shrink-0">
                      {profileData.avatarUrl ? (
                        <img src={profileData.avatarUrl} className="h-full w-full rounded-full object-cover" />
                      ) : (
                        <div className="h-full w-full rounded-full bg-zinc-100 dark:bg-zinc-800 flex items-center justify-center">
                    ...
                    <div className="space-y-1">
                      <p className="text-sm dark:text-zinc-300">
                        <span className="font-bold mr-2 dark:text-zinc-100">{selectedPost.userName}</span>
                        {selectedPost.text}
                      </p>
                    </div>
                  </div>
                ...
                <div className="p-4 border-t border-zinc-100 dark:border-zinc-800 space-y-3">
                  <div className="flex items-center gap-4">
                    <Heart className={`h-6 w-6 ${selectedPost.likeStatus ? 'text-red-500 fill-red-500' : 'dark:text-zinc-100'}`} />
                    <MessageCircle className="h-6 w-6 dark:text-zinc-100" />
                  </div>
                  <p className="text-sm font-bold dark:text-zinc-100">{selectedPost.likesCount} отметок «Нравится»</p>
                </div>
              </div>
            </motion.div>
          </div>
